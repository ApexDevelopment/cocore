...
The wizard's "Attest now" button drives **option-B** directly: it reads the
agent's signing pubkey (`cocore agent pubkey`) and POSTs
`request-attestation {serial, udid, publicKey}`, so the MDM `DeviceInformation`
attestation binds to the signing key. Because Apple captures that attestation
asynchronously and rate-limits it (~1/device/7d), it commonly won't land inside
the wizard's 180s poll — so `chain-not-captured` is shown as a **pending**
("requested, will land in the background") state, not a red failure.

When the attesting step stalls on a genuine fault it classifies what it observed
into a stable code and `NSLog`s a full `co/core Secure Mode attestation failed
[...]` report (serial, enrolled, elapsed/polls, the `request-attestation`
response, the last `attestation-chain` poll). That report is in the user's log
file and (for faults) in the dialog / (for pending) under "Technical detail", so
it's copy-pasteable straight into an issue / a model. Codes:
...
| `secure-mode/push-failed`        | `request-attestation` returned `status=error` / non-200 — NanoMDM enqueue/push failed.                                                                                                                                                                                                | NanoMDM `/v1/enqueue` + `/v1/push`; that the device's UDID is a valid NanoMDM target.                                                                                             |
...
| `secure-mode/chain-not-captured` | Expected first-run outcome (shown as **pending**, not an error). Every poll was a clean 200 `pending`; the option-B request is in but Apple captures the attestation asynchronously + rate-limits it (~1/device/7d), so it rarely lands in the 180s window. The agent's background flow also re-requests ≤ every 6h and picks up the chain on its next refresh. | Agent log for `MDA auto:` lines (request/bind); with `COCORE_MDM_WEBHOOK_DEBUG` set, `GET attestation-chain?serial=zzwebhookdebuglast` to see whether NanoMDM is posting results. |
